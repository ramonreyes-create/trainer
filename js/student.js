...
function dqNow(){ return new Date().toISOString(); }
function dqElapsed(){ return Math.round((Date.now()-sessionStart)/1000); }
function dqActivitySeconds(){ return Math.round((Date.now()-phaseStart)/1000); }
function dqSafeText(v){ return String(v ?? "").replace(/\s+/g," ").trim(); }
function logEvent(type, detail){
  const settings = getSettings ? getSettings() : {};
  activityLog.push({
    timestamp:dqNow(),
    secondsFromStart:dqElapsed(),
    type:type,
    name:settings.name||"",
    course:settings.course||"",
    theme:(typeof getTheme==="function" && document.getElementById('themeSelect')) ? (getTheme()?.title||"") : "",
    mode:(typeof getModeLabel==="function") ? getModeLabel() : "",
    detail:detail||{}
  });
}
function logAnswer(activity, question, answer, correctAnswer, isCorrect){
  logEvent("answer", {
    activity:activity||getModeLabel(),
    question:dqSafeText(question),
    answer:dqSafeText(answer),
    correctAnswer:dqSafeText(correctAnswer),
    correct:!!isCorrect,
    activitySeconds:dqActivitySeconds()
  });
}
function collectOpenText(){
  return [...document.querySelectorAll('.task.open')].map((t,i)=>({
    part: dqSafeText(t.querySelector('h3')?.textContent || ('Teil '+(i+1))),
    text: dqSafeText(t.querySelector('textarea')?.value || '')
  })).filter(x=>x.text);
}
function collectFullProtocol(){
  ...
  return {
    ...
    timeSeconds:dqElapsed(),
    activityTimeSeconds:dqActivitySeconds(),
    errors:collectErrors(),
    focusLossCount:focusLossCount,
    fullscreenExitCount:fullscreenExitCount,
    openText:collectOpenText(),
    protocol:activityLog,
    userAgent:navigator.userAgent
  };
}
window.addEventListener('blur',()=>{focusLossCount++; logEvent('focus_lost',{count:focusLossCount});});
window.addEventListener('focus',()=>logEvent('focus_returned',{}));
document.addEventListener('visibilitychange',()=>logEvent(document.hidden?'tab_hidden':'tab_visible',{}));
document.addEventListener('fullscreenchange',()=>{ if(!document.fullscreenElement){fullscreenExitCount++; logEvent('fullscreen_exit',{count:fullscreenExitCount});} else {logEvent('fullscreen_enter',{});} });
...
function render(){let t=getTheme(),m=document.getElementById('modeSelect').value; phaseStart=Date.now(); currentActivity=getModeLabel(); logEvent('activity_start',{theme:t.title,mode:getModeLabel()});let h=`<section class="card"><span class="badge">${esc(t.title)}</span><h2>${esc(t.title)}</h2></section>`; if(m==="training")h+=training(t); if(m==="families")h+=familiesMode(t); if(m==="prep")h+=prepMode(t); if(m==="memory")h+=memory(t); if(m==="crossword")h+=crossword(t); if(m==="exam")h+=exam(t); if(m==="mini")h+=mini(t); document.getElementById('content').innerHTML=h; updateScore(); if(m==="memory")initMemory(t); if(m==="training")initLetterTasks();}
...
function checkLetters(b){let t=b.closest(".task"),v=t.querySelector(".answerBox").textContent,a=clean(t.dataset.answer),f=t.querySelector(".feedback"),ok=v===a;f.textContent=ok?"Richtig!":"Noch nicht. Richtige Antwort: "+t.dataset.answer;f.className="feedback "+(ok?"correct":"wrong");t.dataset.correct=ok?"1":"0";logAnswer("Buchstaben", t.querySelector(".question")?.textContent||"", v, t.dataset.answer, ok);updateScore()}
...
fillSelect();fillCourseSelect();document.getElementById('studentName').value=localStorage.getItem('dqStudentName')||'';render();logEvent('session_ready',{});