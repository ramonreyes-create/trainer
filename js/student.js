...
let dqSessionId = localStorage.getItem("dqSessionId") || ("dq-" + Date.now() + "-" + Math.random().toString(16).slice(2));
localStorage.setItem("dqSessionId", dqSessionId);
let dqSessionStart = Number(localStorage.getItem("dqSessionStart") || Date.now());
localStorage.setItem("dqSessionStart", String(dqSessionStart));
let dqLog = JSON.parse(localStorage.getItem("dqSessionLog") || "[]");

function nowIso(){ return new Date().toISOString(); }
function currentThemeTitle(){ try { return getTheme()?.title || ""; } catch(e){ return ""; } }
function currentModeValue(){ return document.getElementById("modeSelect")?.value || ""; }
function persistLog(){ localStorage.setItem("dqSessionLog", JSON.stringify(dqLog.slice(-500))); }
...
function getAllOpenTexts(){
  return [...document.querySelectorAll("textarea")].map((x,i)=>({field:i+1, text:x.value.trim()})).filter(x=>x.text);
}
function resetSessionLog(){
  dqSessionId = "dq-" + Date.now() + "-" + Math.random().toString(16).slice(2);
  dqSessionStart = Date.now();
  dqLog = [];
  localStorage.setItem("dqSessionId", dqSessionId);
  localStorage.setItem("dqSessionStart", String(dqSessionStart));
  persistLog();
...
document.addEventListener("visibilitychange",()=>logEvent(document.hidden?"tab_hidden":"tab_visible"));
document.addEventListener("fullscreenchange",()=>logEvent(document.fullscreenElement?"fullscreen_enter":"fullscreen_exit"));


function effectiveConfig(){
  return window.DQ_CONFIG || {appVersion:"8.6-final-functional", schoolName:"DeutschQuest DSD II", sheetUrl:"", courses:["DSD II"], passPercent:80};
}
function saveStudent(){
  ...
  logEvent("student_saved", {name:document.getElementById("studentName").value.trim(), course:document.getElementById("studentCourse").value.trim()});
}
...
async function sendCurrentResults(){
  ...
  logEvent("send_protocol_clicked", {openTexts:getAllOpenTexts()});
  const score=getCurrentScore();
  const payload={
    appVersion:"8.6-final-functional",
    sessionId:dqSessionId,
    sessionStarted:new Date(dqSessionStart).toISOString(),
    ...
    timeSeconds:Math.round((Date.now()-dqSessionStart)/1000),
    phaseTimeSeconds:Math.round((Date.now()-phaseStart)/1000),
    errors:collectErrors(),
    openTexts:getAllOpenTexts(),
    events:dqLog,
  ...
  try{
    ...
    // Nach einem erfolgreichen Versand beginnt intern ein neues Protokoll, damit nichts doppelt gesendet wird.
    localStorage.removeItem("dqSessionLog");
    localStorage.removeItem("dqSessionStart");
    localStorage.removeItem("dqSessionId");
    resetSessionLog();
  }catch(e){
    status.textContent="Senden fehlgeschlagen.";
    status.className="small saveBad";
    logEvent("send_failed", {message:String(e)});
  }
}
...
function render(){let t=getTheme(),m=document.getElementById('modeSelect').value; phaseStart=Date.now(); logEvent("activity_opened", {theme:t.title, mode:m, modeLabel:getModeLabel()}); let h=`<section class="card"><span class="badge">${esc(t.title)}</span><h2>${esc(t.title)}</h2></section>`; if(m==="training")h+=training(t); if(m==="families")h+=familiesMode(t); if(m==="prep")h+=prepMode(t); if(m==="memory")h+=memory(t); if(m==="crossword")h+=crossword(t); if(m==="exam")h+=exam(t); if(m==="mini")h+=mini(t); document.getElementById('content').innerHTML=h; updateScore(); if(m==="memory")initMemory(t); if(m==="training")initLetterTasks();}
...
function checkLetters(b){let t=b.closest(".task"),v=t.querySelector(".answerBox").textContent,a=clean(t.dataset.answer),f=t.querySelector(".feedback"),ok=v===a;f.textContent=ok?"Richtig!":"Noch nicht. Richtige Antwort: "+t.dataset.answer;f.className="feedback "+(ok?"correct":"wrong");t.dataset.correct=ok?"1":"0";logEvent("answer_checked",{activity:"letters",answer:v,expected:t.dataset.answer,correct:ok});updateScore()}
...
function initMemory(t){let pairs=t.words.slice(0,8),cards=[];pairs.forEach((w,i)=>{cards.push({p:i,t:w[0],type:"de"});cards.push({p:i,t:w[1],type:"es"})});cards=shuffle(cards);let grid=document.getElementById("memoryGrid"),open=[],moves=0,matched=0;cards.forEach(c=>{let d=document.createElement("div");d.className="memCard hidden";d.dataset.p=c.p;d.dataset.type=c.type;d.onclick=()=>{if(d.classList.contains("matched")||!d.classList.contains("hidden")||open.length===2)return;d.classList.remove("hidden");d.textContent=c.t;open.push(d);if(open.length===2){moves++;document.getElementById("moves").textContent=moves;setTimeout(()=>{if(open[0].dataset.p===open[1].dataset.p&&open[0].dataset.type!==open[1].dataset.type){open.forEach(x=>x.classList.add("matched"));matched++;document.getElementById("pairs").textContent=matched;logEvent("memory_pair_found",{pairs:matched,moves});if(matched===8)logEvent("activity_completed",{activity:"memory",pairs:matched,moves});}else{open.forEach(x=>{x.classList.add("hidden");x.textContent=""})}open=[]},700)}};grid.appendChild(d)})}
function checkMatch(b){let t=b.closest(".task"),v=t.querySelector("select").value,a=t.dataset.answer,f=t.querySelector(".feedback"),ok=v===a;f.textContent=ok?"Richtig!":"Noch nicht.";f.className="feedback "+(ok?"correct":"wrong");t.dataset.correct=ok?"1":"0";logEvent("answer_checked",{activity:"match",answer:v,expected:a,correct:ok});updateScore()}
function checkFamily(b){let t=b.closest(".task"),i=t.querySelector("input"),f=t.querySelector(".feedback"),ok=norm(i.value)===norm(t.dataset.answer);f.textContent=ok?"Richtig!":"Noch nicht. Richtige Antwort: "+t.dataset.answer;f.className="feedback "+(ok?"correct":"wrong");t.dataset.correct=ok?"1":"0";logEvent("answer_checked",{activity:"families",answer:i.value,expected:t.dataset.answer,correct:ok});updateScore()}
function checkPrep(b){let t=b.closest(".task"),v=t.querySelector("select").value,a=t.dataset.answer,f=t.querySelector(".feedback"),ok=v===a;f.textContent=ok?"Richtig!":"Noch nicht. Richtige Präposition: "+a;f.className="feedback "+(ok?"correct":"wrong");t.dataset.correct=ok?"1":"0";t.querySelector(".prepSolution").style.display="block";logEvent("answer_checked",{activity:"prep",answer:v,expected:a,correct:ok});updateScore()}
function checkLuecken(){let blanks=[...document.querySelectorAll(".inlineBlank.task")];let okCount=0;let answers=[];blanks.forEach(b=>{let inp=b.querySelector("input");let ok=norm(inp.value)===norm(b.dataset.answer);inp.style.borderColor=ok?"#16a34a":"#dc2626";inp.style.background=ok?"#dcfce7":"#fee2e2";b.dataset.correct=ok?"1":"0";answers.push({answer:inp.value,expected:b.dataset.answer,correct:ok});if(ok)okCount++});let f=document.getElementById("lueckenFeedback");f.textContent=okCount===blanks.length?"Sehr gut!":`${okCount}/${blanks.length} richtig.`;f.className="feedback "+(okCount===blanks.length?"correct":"wrong");logEvent("activity_checked",{activity:"lueckentext",correct:okCount,total:blanks.length,answers});updateScore()}
function checkCrossword(){let rows=[...document.querySelectorAll(".crossRow")];let allFilled=rows.every(r=>[...r.querySelectorAll(".letterBox")].every(inp=>inp.value.trim()!==""));let result=document.getElementById("crossResult");document.querySelectorAll(".letterBox").forEach(i=>i.classList.remove("ok","bad"));document.getElementById("showSolutionBtn").style.display="none";if(!allFilled){result.textContent="Bitte zuerst alle Wörter vollständig schreiben.";result.className="feedback wrong";return}let allOk=true;rows.forEach(r=>{let word=r.dataset.word;[...r.querySelectorAll(".letterBox")].forEach((inp,i)=>{let ok=clean(inp.value)===word[i];inp.classList.add(ok?"ok":"bad");if(!ok)allOk=false})});result.textContent=allOk?"Sehr gut! Alle Wörter sind richtig.":"Einige Buchstaben sind noch falsch.";result.className="feedback "+(allOk?"correct":"wrong");logEvent("activity_checked",{activity:"crossword",correct:allOk});document.getElementById("showSolutionBtn").style.display="inline-block"}
...
fillSelect();fillCourseSelect();document.getElementById('studentName').value=localStorage.getItem('dqStudentName')||'';if(!dqLog.length)logEvent('session_started');render();